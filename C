...
int main(void)
{
  ...
  I2C_ScanAddresses();
    ...
    LCD_CLEAR();
    LCD_XY(0, 0);
    LCD_PUTS("STM32 Temp System"); // 첫 줄에 제목 고정
  /* USER CODE END 2 */
  ...
  /* USER CODE BEGIN WHILE */
    while (1)
    {
        ...
        if (HAL_ADC_PollForConversion(&hadc1, 10) == HAL_OK)
        {
            ...
            // 3.3V 전압 기준, 12비트 ADC(4095), LM35(10mV/1도) 공식
            ...
            LCD_XY(0, 1);
            sprintf(lcd_buf, "Temp:%.1f C   ", temperature);
            LCD_PUTS(lcd_buf);
        }
        HAL_ADC_Stop(&hadc1);
        HAL_Delay(1000);
...
static void MX_ADC1_Init(void)
{
  ...
  */
  sConfig.Channel = ADC_CHANNEL_0; // PA0 핀을 사용한다면 CHANNEL_0으로 변경
